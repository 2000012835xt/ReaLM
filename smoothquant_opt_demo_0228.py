...
from smoothquant.fake_quant import W8A8Linear, W8A8BMM, NoisyW8A8Linear, NoisyW8A8BMM, layer_norm_without_outlier
...
class Evaluator:
    ...
    def evaluate(self, model):
        ...
        total, hit = 0, 0
        for batch in tqdm(self.dataset, desc="Evaluating lambada"):
...
class Evaluator_ppl:
    ...
    def evaluate(self, model):
        model.eval()
        nlls = []
        for i in tqdm(range(self.n_samples), desc="Evaluating ppl..."):
...
for i in range(len(err_prob_list)):
    ...
    n_samples=100
    dataset_wikitext = load_dataset("wikitext", "wikitext-2-raw-v1", split="validation")
