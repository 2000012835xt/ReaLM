...
def severity_estimation(result, result_injected, a, b, log2_freq_lower_bound): ## log2(freq)=a*log2(MSD)-b
    # _, fmap_shape1, fmap_shape2=result.shape
    # tile_number_1=int(fmap_shape1/array_size)
    # tile_number_2=int(fmap_shape2/array_size)
    # tiles=result[0].unfold(0,array_size,array_size).unfold(1,array_size,array_size)
    # tiles_injected=result_injected[0].unfold(0,array_size,array_size).unfold(1,array_size,array_size)
    # diff=tiles_injected-tiles
    # MSDs=diff.reshape(tile_number_1,tile_number_2,-1).sum(dim=2)
    # freqs=torch.count_nonzero(diff.reshape(tile_number_1,tile_number_2,-1), dim=2)
    # log2_MSDs=MSDs.abs().log2()
    # log2_freqs=freqs.log2()
    # severe=(log2_freqs<1.273*log2_MSDs-20.076+6*0.273) & (log2_freqs>5.95-6)
    # recompute_ratio=severe.sum()/(tile_number_1*tile_number_2)
    diff=result_injected-result

    MSDs=diff.sum()
    log2_mag_lower_bound=b-(a-1)*MSDs.abs().log2()

    row_checksum=diff.sum(dim=0)
    col_checksum=diff.sum(dim=1)
    row_freq=(row_checksum.abs().log2()>log2_mag_lower_bound).sum()
    col_freq=(col_checksum.abs().log2()>log2_mag_lower_bound).sum()
    freq=torch.max(row_freq,col_freq)
    recompute=freq.log2()>log2_freq_lower_bound
    recompute_MSD=MSDs.abs().log2()>(log2_freq_lower_bound+b)/a
    print('freq=',freq.log2().cpu().item(), 'MSD=',MSDs.abs().log2().cpu().item(), 'recompute=',recompute.cpu().item(), 'recompute_MSD=', recompute_MSD.cpu().item())
    return recompute, recompute_MSD
...
class NoisyW8A8Linear(W8A8Linear):
    ...
    def inject_error(self,y, w_scales, a_scales, err_prob):
        ...
        flip_bit=torch.randint_like(result,0,30, device='cuda')
        ...
        recompute_tensor[i,j], recompute_tensor_MSD[i,j]=severity_estimation(result=result, result_injected=result_injected,
                                                                             a=1.273, b=20.076, log2_freq_lower_bound=5.951)
...
class NoisyW8A8BMM(W8A8BMM):
    ...
    def inject_error(self,y, w_scales, a_scales, err_prob):
        ...
        recompute_tensor[i,j], recompute_tensor_MSD[i,j]=severity_estimation(result=result, result_injected=result_injected,
                                                                             a=1.530, b=24.660, log2_freq_lower_bound=8.04)
...
# pdb.set_trace()
err_prob_list=[1e-8, 1e-7, 2e-7, 5e-7, 1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 2.2e-5, 2.5e-5, 3e-5, 4e-5]
cases=len(err_prob_list)
repeat=1000
recompute_tensor=torch.zeros(cases, repeat, dtype=torch.bool, device='cuda')
recompute_tensor_MSD=torch.zeros(cases, repeat, dtype=torch.bool, device='cuda')

tt=torch.nn.Linear(2048,2048,bias=True,device='cuda')
given_weight=torch.load('fmap/opt_k_weight.pt')
given_activation=torch.load('fmap/opt_k_input.pt')
given_bias=torch.load('fmap/opt_k_bias.pt')
tt.weight.data=given_weight
tt.bias.data=given_bias
data_in=given_activation



for i in range(cases):
    err_prob=err_prob_list[i]
    print(err_prob, torch.log2(torch.tensor(err_prob*2048*2048)))
    for j in range(repeat):
        ...
        # normal=W8A8BMM(act_quant='per_tensor',quantize_output=False)
        # noisy=NoisyW8A8BMM(act_quant='per_tensor', quantize_output=False, err_prob=err_prob)
        
        out_normal=normal(data_in)
        out_noisy=noisy(data_in)

print(recompute_tensor.sum(dim=1))
print(recompute_tensor_MSD.sum(dim=1))
